...
export const getTimeslot =
  (token: string, userData: UserData, date: string) =>
...
export const setAppointmentDateTime =
  (
    token: string,
    userData: UserData,
    patient: Patient,
  ...
  async (dispatch: AppDispatch) => {
    ...
    try {
      ...
      console.log(res.data);
      // console.log(
      //   `userDate.role = ${userData.role}, roles.doctor = ${roles.doctor}`
      // );
      if (userData.role === roles.doctor) {
        dispatch(doctorAcceptPatient(token, patient, userData, doctorComment));
      }
...
export const modifyAppointmentDateTime =
  (
    token: string,
    userData: UserData,
    patientEmail: string,
...
export const doctorAcceptPatient =
  (token: string, patient: Patient, doctor: UserData, doctorComment?: string) =>
  async (dispatch: AppDispatch) => {
    ...
    try {
      console.log("in doctorAcceptPatient");
...
export const markDone =
  (token: string, patient: Patient, expertRole: string) =>
  async (dispatch: AppDispatch) => {
    const endpointPath =
      expertRole == roles.counselor
        ? "/api/v1/counsellor/markCounsellingDone"
        : "/api/v1/doctor/markDoctoringDone";
